...
const char *latestFeatures[] = {
    ...
    "Use syntax `int t = inf.readInt(1, 3, \"~t\");` to skip the lower bound check. Tildes can be used on "
    "either side or both: ~t, t~, ~t~",
    ...
    "Added opt defaults via opt<T>(key/index, default_val); check unused opts when using has_opt or default "
    "opt (turn off this check with suppressEnsureNoUnusedOpt()).",
    "For checker added --group and --testset command line params (like for validator), use checker.group() "
    "or checker.testset() to get values",
    "Added quitpi(points_info, message) function to return with _points exit code 7 and given points_info",
    "rnd.partition(size, sum[, min_part=1]) returns random (unsorted) partition which is a representation of "
    "the given `sum` as a sum of `size` positive integers (or >=min_part if specified)",
    ...
    "Opts supported: use them like n = opt<int>(\"n\"), in a command line you can use an exponential "
    "notation",
    "Reformatted",
    "Use setTestCase(i) or unsetTestCase() to support test cases (you can use it in any type of program: "
    ...
    "Introduced space-separated read functions: readWords/readTokens, multilines read functions: "
    "readStrings/readLines",
    "Introduced space-separated read functions: "
    "readInts/readIntegers/readLongs/readUnsignedLongs/readDoubles/readReals/readStrictDoubles/"
    "readStrictReals",
    ...
    "is good idea to use it in interactors",
...
#if (_WIN32 || __WIN32__ || __WIN32 || _WIN64 || __WIN64__ || __WIN64 || WINNT || __WINNT || __WINNT__ ||    \
     __CYGWIN__)
...
template <typename T> static std::string vtos(const T &t) { return vtos(t, std::is_integral<T>()); }
...
template <typename T> static std::string toHumanReadableString(const T &n) { return vtos(n); }
#endif
...
class random_t {
  ...
  public:
    ...
    int wnext(unsigned int from, unsigned int to, int type) {
        if (from > to)
            __testlib_fail(
                "random_t::wnext(unsigned int from, unsigned int to, int type): from can't not exceed to");
    ...
    long long wnext(long long from, long long to, int type) {
        if (from > to)
            __testlib_fail(
                "random_t::wnext(long long from, long long to, int type): from can't not exceed to");
        return wnext(to - from + 1, type) + from;
    }
    ...
    double wnext(double from, double to, int type) {
        if (from >= to)
            __testlib_fail(
                "random_t::wnext(double from, double to, int type): from should be strictly less than to");
    ...
    /* Returns random (unsorted) partition which is a representation of sum as a sum of integers not less than
     * min_part. */
    template <typename T> std::vector<T> partition(int size, T sum, T min_part) {
        ...
        if (*std::min_element(result.begin(), result.end()) < min_part)
            __testlib_fail(
                "random_t::partition: partition min is expected to be no less than the given min_part");
    ...
    template <typename T> std::vector<T> partition(int size, T sum) { return partition(size, sum, T(1)); }
};
  ...
  public:
    ...
    void setTestCase(int testCase) {
        if (testCase < 0 || testCase > __TESTLIB_MAX_TEST_CASE)
            __testlib_fail(testlib_format_("testCase expected fit in [1,%d], but %d doesn't",
...
struct InStream {
    ...
    std::vector<unsigned long long> readUnsignedLongs(int size, unsigned long long minv,
    ...
    /* Reads space-separated sequence of integers. */
    std::vector<int> readInts(int size, int minv, int maxv, const std::string &variablesName = "",
                              int indexBase = 1);
    ...
    std::vector<std::string> readStrings(int size, const std::string &ptrn,
                                         const std::string &variableName = "", int indexBase = 1);
...
struct ValidatorBoundsHit {
    ...
    ValidatorBoundsHit merge(const ValidatorBoundsHit &validatorBoundsHit, bool ignoreMinBound,
                             bool ignoreMaxBound) {
        return ValidatorBoundsHit(__testlib_max(minHit, validatorBoundsHit.minHit) || ignoreMinBound,
...
class Validator {
  ...
  public:
    ...
    std::string testset() const {
        if (!_initialized)
            __testlib_fail("Validator should be initialized with registerValidation(argc, argv) instead of "
    ...
    void addBoundsHit(const std::string &variableName, ValidatorBoundsHit boundsHit) {
        if (isVariableNameBoundsAnalyzable(variableName) &&
            ...
            _boundsHitByVariableName[preparedVariableName] =
                boundsHit.merge(_boundsHitByVariableName[preparedVariableName], ignoreMinBound(variableName),
                                ignoreMaxBound(variableName));
        }
    }
    ...
    template <typename T> void adjustConstantBounds(const std::string &variableName, T lower, T upper) {
        if (isVariableNameBoundsAnalyzable(variableName) &&
    ...
    void writeTestOverviewLog() {
        if (!_testOverviewLogFileName.empty()) {
            ...
            if (fileName == "stdout")
                ...
                if (NULL == f)
                    __testlib_fail("Validator::writeTestOverviewLog: can't write test overview log to (" +
                                   fileName + ")");
            }
            fprintf(f, "%s%s%s%s", getBoundsHitLog().c_str(), getFeaturesLog().c_str(),
                    getConstantBoundsLog().c_str(), getVariablesLog().c_str());
            std::fflush(f);
            if (!standard_file)
                if (std::fclose(f))
                    __testlib_fail("Validator::writeTestOverviewLog: can't close test overview log file (" +
                                   fileName + ")");
        }
    }
        ...
        if (!_testMarkupFileName.empty()) {
            ...
            if (!readChars.empty()) {
                ...
                if (_testMarkupFileName == "stdout")
                    ...
                    if (NULL == f)
                        __testlib_fail("Validator::writeTestMarkup: can't write test markup to (" +
                                       _testMarkupFileName + ")");
                }
                ...
                if (!standard_file)
                    if (std::fclose(f))
                        __testlib_fail("Validator::writeTestMarkup: can't close test markup file (" +
                                       _testCaseFileName + ")");
            }
        }
    }
            ...
            if (!readChars.empty()) {
                ...
                if (!testCaseContent.empty()) {
                    ...
                    if (_testCaseFileName.empty() || _testCaseFileName == "stdout")
                        ...
                        if (NULL == f)
                            __testlib_fail("Validator::writeTestCase: can't write test case to (" +
                                           _testCaseFileName + ")");
                    }
                    ...
                    if (!standard_file)
                        if (std::fclose(f))
                            __testlib_fail("Validator::writeTestCase: can't close test case file (" +
                                           _testCaseFileName + ")");
                }
            }
        }
...
struct TestlibFinalizeGuard {
    ...
    ~TestlibFinalizeGuard() {
        ...
        if (_alive) {
            ...
            if (!registered)
                __testlib_fail("Call register-function in the first line of the main (registerTestlibCmd or "
                               "other similar)");
        }
...
NORETURN void InStream::quit(TResult result, const char *msg) {
    ...
    if (message.length() > maxMessageLength) {
        std::string warn =
            "message length exceeds " + vtos(maxMessageLength) + ", the message is truncated: ";
    ...
    if (mode != _output && result != _fail) {
        if (mode == _input && testlibMode == _validator && lastLine != -1)
            quits(_fail,
                  __testlib_appendMessage(__testlib_appendMessage(message, name), "line " + vtos(lastLine)));
        else
    ...
    if (!p.matches(_tmpReadToken)) {
        if (readManyIteration == NO_INDEX) {
            ...
            if (variableName.empty())
                ...
                quit(_wa,
                     ("Token element " + variableName + "[" + vtos(readManyIteration) + "] equals to \"" +
                      __testlib_part(_tmpReadToken) + "\", doesn't correspond to pattern \"" + p.src() + "\"")
                         .c_str());
        }
    }
...
static inline double stringToDouble(InStream &in, const std::string &buffer) {
    for (size_t i = 0; i < buffer.length(); i++)
        if (buffer[i] == '\0')
            in.quit(
                _pe,
                ("Expected double, but \"" + __testlib_part(buffer) + "\" found (it contains \\0)").c_str());
...
static inline long long stringToLongLong(InStream &in, const std::string &buffer) {
    for (size_t i = 0; i < buffer.length(); i++)
        if (buffer[i] == '\0')
            in.quit(
                _pe,
                ("Expected integer, but \"" + __testlib_part(buffer) + "\" found (it contains \\0)").c_str());
...
long long InStream::readLong(long long minv, long long maxv, const std::string &variableName) {
    ...
    if (result < minv || result > maxv) {
        if (readManyIteration == NO_INDEX) {
            ...
            if (variableName.empty())
                quit(_wa, ("Integer element [index=" + vtos(readManyIteration) + "] equals to " +
                           vtos(result) + ", violates the range [" + toHumanReadableString(minv) + ", " +
                           toHumanReadableString(maxv) + "]")
                              .c_str());
            else
                quit(_wa, ("Integer element " + std::string(variableName) + "[" + vtos(readManyIteration) +
...
std::vector<long long> InStream::readLongs(int size, long long minv, long long maxv,
                                           const std::string &variablesName, int indexBase) {
    if (strict && !variablesName.empty())
        validator.addVariable(variablesName);
...
unsigned long long InStream::readUnsignedLong(unsigned long long minv, unsigned long long maxv,
    ...
    if (result < minv || result > maxv) {
        if (readManyIteration == NO_INDEX) {
            if (variableName.empty())
                quit(_wa, ("Unsigned integer " + vtos(result) + " violates the range [" +
                           toHumanReadableString(minv) + ", " + toHumanReadableString(maxv) + "]")
                              .c_str());
            else
            ...
            if (variableName.empty())
                ...
                quit(_wa,
                     ("Unsigned integer element " + std::string(variableName) + "[" +
                      vtos(readManyIteration) + "] equals to " + vtos(result) + ", violates the range [" +
                      toHumanReadableString(minv) + ", " + toHumanReadableString(maxv) + "]")
                         .c_str());
        }
    }
...
std::vector<unsigned long long> InStream::readUnsignedLongs(int size, unsigned long long minv,
                                                            unsigned long long maxv,
                                                            const std::string &variablesName, int indexBase) {
    if (strict && !variablesName.empty())
        validator.addVariable(variablesName);
...
std::vector<int> InStream::readInts(int size, int minv, int maxv, const std::string &variablesName,
                                    int indexBase) {
    if (strict && !variablesName.empty())
        validator.addVariable(variablesName);
...
std::vector<int> InStream::readInts(int size,
                                    int indexBase){__testlib_readMany(readInts, readInt(), int, true)}

std::vector<int> InStream::readIntegers(int size, int minv, int maxv, const std::string &variablesName,
                                        int indexBase) {
    if (strict && !variablesName.empty())
        validator.addVariable(variablesName);
...
double InStream::readReal(double minv, double maxv, const std::string &variableName) {
    ...
    if (result < minv || result > maxv) {
        if (readManyIteration == NO_INDEX) {
            if (variableName.empty())
                ...
                quit(_wa, ("Double parameter [name=" + std::string(variableName) + "] equals to " +
                           vtos(result) + ", violates the range [" + vtos(minv) + ", " + vtos(maxv) + "]")
                              .c_str());
        } else {
            if (variableName.empty())
                quit(_wa, ("Double element [index=" + vtos(readManyIteration) + "] equals to " +
                           vtos(result) + ", violates the range [" + vtos(minv) + ", " + vtos(maxv) + "]")
                              .c_str());
            else
                quit(_wa, ("Double element " + std::string(variableName) + "[" + vtos(readManyIteration) +
                           "] equals to " + vtos(result) + ", violates the range [" + vtos(minv) + ", " +
                           vtos(maxv) + "]")
                              .c_str());
        }
    }
    ...
    if (strict && !variableName.empty()) {
        validator.addBoundsHit(variableName,
                               ValidatorBoundsHit(doubleDelta(minv, result) < ValidatorBoundsHit::EPS,
...
double InStream::readStrictReal(double minv, double maxv, int minAfterPointDigitCount,
    ...
    if (result < minv || result > maxv) {
        if (readManyIteration == NO_INDEX) {
            ...
            if (variableName.empty())
                quit(_wa, ("Strict double element [index=" + vtos(readManyIteration) + "] equals to " +
                           vtos(result) + ", violates the range [" + vtos(minv) + ", " + vtos(maxv) + "]")
                              .c_str());
            else
...
std::vector<double> InStream::readStrictReals(int size, double minv, double maxv, int minAfterPointDigitCount,
    ...
    __testlib_readMany(
        readStrictReals,
        readStrictReal(minv, maxv, minAfterPointDigitCount, maxAfterPointDigitCount, variablesName), double,
        true)
}
...
std::vector<double> InStream::readStrictDoubles(int size, double minv, double maxv,
    ...
    __testlib_readMany(
        readStrictDoubles,
        readStrictDouble(minv, maxv, minAfterPointDigitCount, maxAfterPointDigitCount, variablesName), double,
        true)
}
...
void InStream::readStringTo(std::string &result, const pattern &p, const std::string &variableName) {
    ...
    if (!p.matches(result)) {
        if (readManyIteration == NO_INDEX) {
            if (variableName.empty())
                quit(_wa, ("Line \"" + __testlib_part(result) + "\" doesn't correspond to pattern \"" +
                           p.src() + "\"")
                              .c_str());
            else
            ...
            if (variableName.empty())
                ...
                quit(_wa, ("Line element " + std::string(variableName) + "[" + vtos(readManyIteration) +
...
std::vector<std::string> InStream::readStrings(int size, const std::string &ptrn,
                                               const std::string &variablesName, int indexBase) {
    pattern p(ptrn);
    if (strict && !variablesName.empty())
        validator.addVariable(variablesName);
...
NORETURN void __testlib_help() {
    ...
    std::fprintf(stderr, "    [--testset testset] [--group group] <input-file> <output-file> <answer-file> "
                         "[<report-file> [<-appes>]]\n\n");
        ...
        if (!strcmp("--testCase", argv[i])) {
            if (i + 1 < argc) {
                ...
                if (testCase < 1 || testCase >= __TESTLIB_MAX_TEST_CASE)
                    quit(_fail, testlib_format_("Argument testCase should be between 1 and %d, but ",
...
class Checker {
  ...
  public:
    ...
    std::string testset() const {
        if (!_initialized)
            __testlib_fail("Checker should be initialized with registerTestlibCmd(argc, argv) instead of "
...
void registerTestlibCmd(int argc, char *argv[]) {
    ...
    if (argc < 4 || argc > 6) {
        quit(_fail, std::string("Program must be run with the following arguments: ") +
                        std::string("[--testset testset] [--group group] <input-file> <output-file> "
...
#define ensure_ext(cond)                                                                                     \
    __testlib_ensure((cond), "Line " STRINGIZE(__LINE__) ": Condition failed: \"" #cond "\"")
...
NORETURN void __testlib_expectedButFound(TResult result, std::string expected, std::string found,
    ...
    std::string message;
    if (strlen(prepend) != 0)
        message = testlib_format_("%s: expected '%s', but found '%s'", compress(prepend).c_str(),
                                  compress(expected).c_str(), compress(found).c_str());
    else
        message = testlib_format_("expected '%s', but found '%s'", compress(expected).c_str(),
...
template <class T> struct is_iterator<T, typename __testlib_enable_if<std::is_array<T>::value>::type> {
...
template <typename A> void println(const A *a, const A *b) { __println_range(a, b); }
...
template <typename T> T optValueToIntegral(const std::string &s, bool nonnegative);
...
template <typename T> T optValueToIntegral(const std::string &s_, bool nonnegative) {
...
template <typename T, typename U> T opt(std::false_type /*is_integral*/, U /*is_unsigned*/, int index) {
    return opt<T>(std::is_floating_point<T>(), index);
}

template <typename T> T opt(std::true_type /*is_integral*/, std::false_type /*is_unsigned*/, int index) {
    return optValueToIntegral<T>(__testlib_indexToArgv(index), false);
}

template <typename T> T opt(std::true_type /*is_integral*/, std::true_type /*is_unsigned*/, int index) {
    return optValueToIntegral<T>(__testlib_indexToArgv(index), true);
}

template <> bool opt(std::true_type /*is_integral*/, std::true_type /*is_unsigned*/, int index) {
 ...
 */
template <typename T> T opt(int index) { return opt<T>(std::is_integral<T>(), std::is_unsigned<T>(), index); }
...
template <typename T> T opt(std::false_type is_floating_point, const std::string &key);

template <> std::string opt(std::false_type /*is_floating_point*/, const std::string &key) {
    return __testlib_keyToOpts(key);
}

template <typename T> T opt(std::true_type /*is_integral*/, const std::string &key) {
    return T(optValueToLongDouble(__testlib_keyToOpts(key)));
}

template <typename T, typename U> T opt(std::false_type /*is_integral*/, U, const std::string &key) {
...
template <> bool opt(std::true_type /*is_integral*/, std::true_type /*is_unsigned*/, const std::string &key) {
    if (!has_opt(key))
        return false;
...
void suppressEnsureNoUnusedOpts() { __testlib_ensureNoUnusedOptsSuppressed = true; }
