...
template <typename T = long long> T gen_num(T l = -INF<T>, T r = INF<T>) {
...
template <typename T = long long>
vector<T> gen_unique_array(int len = 0, T minRange = -INF<T>, T maxRange = INF<T>,
...
template <typename T = long long> string gen_big_int(int len = 0, char l = '0', char r = '9') {
    ...
    string str(len, '0');
    for (char &ch : str)
        ch += gen_num<T>(l - '0', r - '0');
    if (str.front() == '0')
        str.front() += gen_num<T>(1, r - '0');
    return str;
}

// Generate an array of pairs with optional interval constraints
template <typename T = long long>
vector<Pair<T>> gen_array_of_pairs(int len = 0, T minRange = -INF<T>, T maxRange = INF<T>,
                                   bool interval = false) {
    assert(len >= 0 && len <= 5e6);
...
template <typename T = long long> vector<Pair<T>> gen_unique_pairs(int n, int pairCount) {
    ...
    while ((int)unique_pairs.size() < pairCount) {
        ...
        T b = gen_num<T>(1, n);
        if (a != b)
            unique_pairs.insert(minmax(a, b));
    }
...
vector<string> gen_substrings(const string &source, int substringCount, int minLength, int maxLength) {
    ...
    while (sz(substrings) < substringCount) {
        ...
        int s = gen_num<int>(0, sz(source) - len);
        substrings.push_back(source.substr(s, len));
    }
    return substrings;
}
